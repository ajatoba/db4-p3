...
function checkSelect(field) {
	if (field.name == 'notrequired' || field.className.indexOf('notrequired') >= 0) {
		return true;
	}
...
function checkRadio(field) {
	...
	for ( var i = 0, input; input = inputs[i]; i++) {
		if (input.name == field.name) {
			...
			if ( input.className.match('notrequired') ) {
				return true;
			}
...
function checkCheckbox(field) {
	...
	for ( var i = 0, input; input = inputs[i]; i++) {
		if (input.name == field.name) {
			...
			if (input.checked == true || input.name == 'notrequired'
					|| input.className == 'notrequired') {
				return true;
			}
		}
	}
...
function normalizeValidationAttributes(frm) {
  ...
  for (var i = frm.elements.length - 1; i >= 0; i--) {
    ...
    if (className.indexOf('notrequired') == -1) el.setAttribute('required', true);
    // TODO
  }
}